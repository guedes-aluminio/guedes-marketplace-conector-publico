...
async function apagarFonteDuplicada() {
  const SOURCE_ID = 54617;

  const resposta = await fetch(
    `https://${SUBDOMINIO}.kommo.com/api/v4/sources/${SOURCE_ID}`,
    {
      method: "DELETE",
      headers: {
  ...
  console.log("resposta:", texto || "(vazio, normal para DELETE com sucesso)");
}

apagarFonteDuplicada().catch((e) => {
